...
J = (1 / m) * sum(sum(-y_matrix .* log(a3) - (ones(m, num_labels) - y_matrix) .* log(ones(m, num_labels) - a3), 1)); 
%
...
delta3 = a3 - y_matrix; % m x r
DELTA2 = delta3' * a2;  % r x h+1
Theta2_grad = 1/m * DELTA2; % r x h+1

delta2 = delta3 * Theta2(:,2:end) .* sigmoidGradient(z2); % m x h
DELTA1 = delta2' * a1; % h x n 
...
J_reg = (lambda/(2*m)) * (sum(sum(temp1 .* temp1)) + sum(sum(temp2 .* temp2)));
J = J + J_reg;
